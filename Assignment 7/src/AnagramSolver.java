	...
	{	
		...
		else
		{
			...
			while(it.hasNext())
			{
				...
				// Check to make sure that word from dictionary can be formed from the
				// target string's letter inventory
				if(diff != null)
				{
					// Add possible word to smaller list
					out.add(str);
					// Recursive call with reduced letter inventory
					output = getAnagramsHelper(diff, max, dictRed, out, output);
					// Remove possible word from smaller list
					out.remove(str);
